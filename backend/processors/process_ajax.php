...
switch ($case) {
  ...
  case 'list':
    ...
    if (isset($param->table)) {
      ...
      $displayFields      = $param->display_fields;
      $param->table_fields  = $generic->getTableFields($param->table);
      foreach ($displayFields as $k => $v2) {
        $column[$k]   = strtolower($v2->column);
        $coldesc[$k]  = $displayFields[$k]->description = !empty($v2->description) ? $v2->description : '';
        $element[$k]  = $displayFields[$k]->component = !empty($v2->component) ? $v2->component : '';
        $actions[$k]  = $displayFields[$k]->action = !empty($v2->action) ? $v2->action : '';
        $source[$k]   = $displayFields[$k]->source = !empty($v2->source) ? $v2->source : '';
        $class[$k]    = $displayFields[$k]->class = isset($v2->class) ? $v2->class : '';
        $filter[$k]   = $displayFields[$k]->filter = !empty($v2->filter) ? $v2->filter : '';
        $displayFields[$k]->table = !empty($v2->table) ? $v2->table : '';
        $displayFields[$k]->index = $k;
      }
      ...
      if (!empty($col)) {
        ...
        // Store all External tables in the memory for this particular request
        $externalTables = array_filter($displayFields, function ($field) {
          return !empty($field->table);
        });
        if (count($externalTables)) {
          $groupedTables = [];
          foreach ($externalTables as $field) {
            $groupedTables[$field->table][] = $field->column;
            $groupedTables[$field->table][] = $field->actioncol;
          }
          foreach ($groupedTables as $table => $tablecols) {
            $_ = [];
            $__ = implode(",", array_unique($tablecols));
            $q = $db->query("SELECT {$__} FROM {$table}");
            while ($___ = $q->fetch_assoc()) {
              $_[] = $___;
            }
            $groupedTables[$table] = $_;
          };
        }
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          foreach ($row as $colname => $colvalue) {
            ...
            $field = array_filter($displayFields, function ($field) use ($colname) {
              return $field->column == $colname;
            });
            if (!empty($field)) {
              $field = reset($field);

              if (empty($field->table)) {
                $colvalue = displayFieldActions($colvalue, $row, $field->action, $field->source, $paramControl, $param, $field->table);
              }
              $r["c"][$field->index] = utf8ize($colvalue);
            }
          }
          // see($r["c"]);
          if (count($externalTables)) {
            // see($groupedTables);
            foreach ($externalTables as $key => $field) {
              $values = array_values(
                array_map(
                  function ($found) use ($field) {
                    return $found[$field->actioncol];
                  },
                  array_filter($groupedTables[$field->table], function ($thisrow) use ($field, $row, $param) {
                    return $thisrow[$field->column] == $row[$param->primary_key];
                  })
                )
              );
              if ($field->action == "count") {
                $colvalue = count($values);
              } else if ($field->action == "sum") {
                $colvalue = array_sum($values);
              }
              $r["c"][$field->index] = $colvalue;
            }
          }
        ...
        // Get total of data in the table for this request
