...
function setCookies(obj) {
  if (!obj.image) obj.image = site.backend + "/images/default.png";
  var d = new Date();
...
$("[data-clipboard-text]").click(function (e) {
  let text;
  if ($(this).data("clipboard-text")) text = $(this).data("clipboard-text");
  else if ($(this).data("clipboard-from")) text = $(`#${$(this).data("clipboard-from")}`).text();
  var $temp = $("<input>");
  $("body").append($temp);
