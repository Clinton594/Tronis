...
class Auth extends Generic
{
	...
	public function __construct($get_user, $param_name = "admin_signin")
	{
		...
		if ($get_user !== null) {
			...
			$user_id_cols 	= array_extract($param, $identity_columns, false, false);
			...
			if (gettype($get_user) == "array") {
				...
				$user_array = $get_user;
				$get_user = array_extract($user_array, $user_id_cols, false, false);
	...
	public function user($get_user = null)
	{
		...
		if (empty(self::$user) && !empty($identity_val)) {
			...
			if (count($user)) {
				...
				$uri = $this->getURIdata();
				$user->image_col 	= file_path($user->image_col, $uri->backend . "images/c_icon.png");
				self::$user = $user;
			} else {
				$user = null;
			}
	...
	public function login($data)
	{
		...
		if (!empty($param)) {
			...
			$login_password = array_extract((array)$post, [$param->password_col], false, false)[0];
			$user 					= $this->user();
			...
			if (!empty($user)) {
				...
				if ($passwordVerified === true) {
					...
					$data				= array_extract((array)$user, $required_cols, false);
					// see($data);
					if (!empty($param->callback)) $response = call_user_func_array($param->callback, [object($data)]);
					$response 			= (object)array_remap($data, $param);

					$_SESSION['loggedin'] = true;
					$response->status = 1;
					$response->primary_key = $response->{$param->primary_key};
