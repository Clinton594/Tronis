...
class Generic extends DBCred
{
  ...
  public function connect($buildConstants = true)
  {
    if (!$this->isConnected) {
      ...
      $db_handle = new MySQLi("localhost", get_env("DB_USER"), get_env("DB_PASSWORD"), get_env("DB_NAME"));
  ...
  function buildAllConstants($createOnly = false)
  {
    ...
    if ($fields) $this->defineConstants($fields);
  }
  ...
  public function defineConstants($fields)
  {
    foreach ($fields as $name) {
      ...
      $DESC = $CONST . "_DESC";
      $ASC = $CONST . "_ASC";
      $this->{$DESC} = strtolower($CONST . " DESC");
      $this->{$ASC} = strtolower($CONST . " ASC");
    }
  }
  ...
  public function getURIdata($str = "")
  {
    global $_SERVER;
    $server = $this->getServer();
    $link = empty($str) ? $_SERVER['REQUEST_URI'] : $str;

    $request_uri = str_replace('https://', '', $link);
    ...
    $uri = (object)[
      ...
      "domain" => $server,
      "site" => $this->domain,
      "backend" => ($this->isReact && !$this->islocalhost()) ? $this->domain : $this->domain . $this->backend,
      "request_uri" =>  $_SERVER['REQUEST_URI']
    ];
    $backend = ($this->isReact && !$this->islocalhost()) ? "" : $this->backend;
    setcookie("siteData", "{$uri->site},{$backend}," . absolute_filepath("{$uri->site}"), time() + 36000, "/");
    return ($uri);
  }
      ...
      if ($this->has_db && $this->isConnected) {
        ...
        if ($query && $row = $query->fetch_object()) {
          ...
          if (empty($logo)) { //Not an array
            if (empty($row->logo_ref)) $row->logo_ref = "{$uri->backend}images/logo.png";
          }
